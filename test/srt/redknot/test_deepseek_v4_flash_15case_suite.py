...
SPEC = importlib.util.spec_from_file_location(
    "redknot_release_entry", HERE / "benchmark_RedKnot_DeepSeekV4Flash.py"
)
...
class TestRelease15CaseSuites(unittest.TestCase):
                    ...
                    pairs.append(
                        {
                            ...
                            "dense_output": f"dense-{case['suite_case_index']}",
                            "redknot_output": f"redknot-{case['suite_case_index']}",
                            "dense_output_tokens": 1,
    ...
    def test_default_dispatches_256k_then_440k(self) -> None:
        with tempfile.TemporaryDirectory() as tmp, mock.patch.object(
            ENTRY, "HERE", Path(tmp)
        ), mock.patch.object(ENTRY.subprocess, "run") as run:
            run.return_value.returncode = 0
            status = ENTRY._run_default_release_suites()
            summaries = list(Path(tmp).glob("results/deepseek_v4_flash/*/two_length_summary.json"))
            self.assertEqual(len(summaries), 1)
            summary = json.loads(summaries[0].read_text(encoding="utf-8"))
        self.assertEqual(status, 0)
        self.assertEqual(summary["ordered_lengths"], ["256K", "440K"])
        self.assertEqual(run.call_count, 2)
        commands = [call.args[0] for call in run.call_args_list]
        self.assertIn("release_256k_15case.jsonl", " ".join(commands[0]))
        self.assertIn("release_440k_15case.jsonl", " ".join(commands[1]))
