...
if [[ -n "$qualification_profile" ]]; then
  if [[ ! -r "$qualification_profile" ]]; then
    printf 'ERROR: QUALIFICATION_PROFILE is not readable: %q\n' \
...
restart_holder() {
  ...
  cd "$holder_cwd" || return
  setsid nohup "$holder_python" gpu_hold.py \
    > "$run_dir/gpu_holder.log" 2>&1 < /dev/null &
  new_holder=$!
  for _ in $(seq 1 90); do
    if kill -0 "$new_holder" 2>/dev/null; then
      new_pgid=$(ps -o pgid= -p "$new_holder" 2>/dev/null | tr -d ' ')
      new_sid=$(ps -o sid= -p "$new_holder" 2>/dev/null | tr -d ' ')
      active_gpu_pids=$(gpu_pids)
      gpu_count=$(printf '%s\n' "$active_gpu_pids" | sed '/^$/d' | wc -l | tr -d ' ')
      same_group=1
      while IFS= read -r gpu_pid; do
        [[ -z "$gpu_pid" ]] && continue
        gpu_pgid=$(ps -o pgid= -p "$gpu_pid" 2>/dev/null | tr -d ' ')
        [[ "$gpu_pgid" == "$new_holder" ]] || same_group=0
      done <<< "$active_gpu_pids"
      if [[ "$new_pgid" == "$new_holder" && "$new_sid" == "$new_holder" \
            && "$gpu_count" == 8 && "$same_group" == 1 ]]; then
        printf 'holder_restarted pid=%s workers=%s\n' \
          "$new_holder" "$gpu_count" \
...
trap restart_holder EXIT INT TERM
...
holder_exe=$(readlink -f "/proc/$holder_pid/exe")
expected_holder_exe=$(readlink -f "$holder_python")
holder_script=$(tr '\0' '\n' < "/proc/$holder_pid/cmdline" | sed -n '2p')
holder_actual_cwd=$(readlink -f "/proc/$holder_pid/cwd")
holder_pgid=$(ps -o pgid= -p "$holder_pid" | tr -d ' ')
if [[ "$holder_exe" != "$expected_holder_exe" \
      || "$holder_script" != "gpu_hold.py" \
      || "$holder_actual_cwd" != "$holder_cwd" \
         || "$holder_pgid" != "$holder_pid" ]]; then
  printf 'ERROR: holder identity mismatch pid=%s pgid=%s cwd=%s exe=%s script=%q cmd=%q\n' \
    "$holder_pid" "$holder_pgid" "$holder_actual_cwd" "$holder_exe" \
    "$holder_script" "$holder_cmd"
  exit 91
fi
printf 'holder_retained_during_cpu_prelaunch pid=%s pgid=%s\n' \
...
# incidental early CUDA initialization.  The EXIT trap always replaces this
# bootstrap process with the original full holder.
...
for _ in $(seq 1 90); do
  if kill -0 "$holder_pid" 2>/dev/null; then
    ...
    while IFS= read -r gpu_pid; do
      [[ -z "$gpu_pid" ]] && continue
      gpu_pgid=$(ps -o pgid= -p "$gpu_pid" 2>/dev/null | tr -d ' ')
      [[ "$gpu_pgid" == "$holder_pgid" ]] || same_group=0
    done <<< "$active_gpu_pids"
    if [[ "$gpu_count" == 8 && "$same_group" == 1 ]]; then
      bootstrap_ready=1
      break
    fi
  fi
...
if [[ "$bootstrap_ready" != 1 ]]; then
  printf 'ERROR: light bootstrap holder verification failed pid=%s\n' \
    "$holder_pid"
  exit 99
fi
printf 'light_bootstrap_holder_ready pid=%s workers=8 mem_frac=0.08\n' \
  "$holder_pid"
...
setsid env \
  ...
  REDKNOT_HARDWARE_PROFILE="${REDKNOT_HARDWARE_PROFILE:-h200}" \
  ...
  "$runtime_python" test/srt/redknot/utils/benchmark_dsv4_redknot_http.py \
    --mode redknot \
...
for rank in $(seq 0 7); do
  ...
  if [[ -z "$server_pgid" ]]; then
    server_pgid="$this_pgid"
  elif [[ "$this_pgid" != "$server_pgid" ]]; then
...
if ! kill -0 "$holder_pid" 2>/dev/null; then
  printf 'ERROR: holder disappeared during CPU prelaunch pid=%s\n' "$holder_pid"
  exit 97
fi
...
sync "$worker_go_tmp"
mv "$worker_go_tmp" "$worker_go_file"
