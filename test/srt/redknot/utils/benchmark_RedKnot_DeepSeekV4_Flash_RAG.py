...
Examples:
  ...
  CUDA_VISIBLE_DEVICES=0 REDKNOT_N_SAMPLES=1 REDKNOT_LENGTHS=8K \
    python test/srt/redknot/benchmark_RedKnot_DeepSeekV4_Flash_RAG.py
...
IH_CHECKPOINT_MAX_ISLANDS = int(
    os.environ.get("REDKNOT_IH_CHECKPOINT_MAX_ISLANDS", "8")
)
IH_ACTIVE_BUDGET_RATIO = float(os.environ.get("REDKNOT_IH_ACTIVE_BUDGET_RATIO", "0.10"))
IH_QUERY_PROTECTION_TOKENS = int(
    os.environ.get("REDKNOT_IH_QUERY_PROTECTION_TOKENS", "8192")
)
...
_IH_GENERALIZED_STRONG_ACTIVE_RATIO = float(
    os.environ.get("REDKNOT_IH_GENERALIZED_STRONG_ACTIVE_RATIO", "0.15")
)
_IH_GENERALIZED_MEDIUM_ACTIVE_RATIO = float(
    os.environ.get("REDKNOT_IH_GENERALIZED_MEDIUM_ACTIVE_RATIO", "0.20")
)
_IH_GENERALIZED_DIFFUSE_ACTIVE_RATIO = float(
    os.environ.get("REDKNOT_IH_GENERALIZED_DIFFUSE_ACTIVE_RATIO", "0.25")
)
...
IH_STRICT_PERFORMANCE_CLAIMS = (
    os.environ.get("REDKNOT_IH_STRICT_PERFORMANCE_CLAIMS", "1") == "1"
)
...
# Default to the historical fixed sweep for directly comparable formal runs.
# A caller may explicitly freeze a smaller supportability sweep (for example
# ``1`` first, then ``1,2``) and the selected tuple is embedded in the result.
...
IH_QPS_CONCURRENCIES = _ih_parse_qps_concurrencies(
    os.environ.get("REDKNOT_IH_QPS_CONCURRENCIES", "1,2,4,8,16")
)
...
if IH_MLA_OFF_GLOBAL_ATTN_IMPL not in {
    "triton_h1",
    "padded_flashmla_h64",
}:
    raise ValueError(
        "REDKNOT_MLA_OFF_GLOBAL_ATTN_IMPL must be triton_h1 or "
        "padded_flashmla_h64"
    )
if IH_MLA_OFF_GLOBAL_ATTN_IMPL != "triton_h1" and not IH_MLA_OFFLOAD:
    raise ValueError("padded global attention requires pure MLA-off")
...
if not 0 <= IH_MLA_OFF_RESTORE_PIPELINE_GROUP_LAYERS <= 37:
    raise ValueError(
        "REDKNOT_IH_MLA_OFF_RESTORE_PIPELINE_GROUP_LAYERS must be in [0, 37]"
    )
...
IH_MIN_GPU_FREE_MIB = int(
    os.environ.get("REDKNOT_IH_MIN_GPU_FREE_MIB", "70000")
)
...
IH_SERVER_LOG = os.environ.get("REDKNOT_IH_SERVER_LOG", "/tmp/redknot_ih_server.log")
IH_RANK_LOG_DIR = str(
    Path(
        os.environ.get(
            "REDKNOT_IH_RANK_LOG_DIR",
            (
                "/tmp/ranklogs_redknot_mla"
                if IH_NO_LAUNCH
                else f"/tmp/redknot_ih_ranklogs_{os.getpid()}"
            ),
        )
    )
    .expanduser()
...
IH_SERVER_SCRIPT = os.environ.get(
    "REDKNOT_IH_SERVER_SCRIPT",
    str(REPO / "server" / "start_server_redknot_mla.sh"),
)
IH_VENV_PY = os.environ.get(
    "REDKNOT_IH_VENV_PY", str(REPO / ".venv_tf5" / "bin" / "python")
)
...
PROFILE_EXPECTED_HEADS = int(
    os.environ.get("REDKNOT_MLA_PROFILE_EXPECTED_HEADS", "64")
)
...
def _merge_tp_profile_outputs() -> None:
    """Merge contiguous TP query-head shards into one 64-head profile."""
    if TP_SIZE <= 1:
        return
...
_DRIFT_MANIFEST_FORMAT = "redknot_dsv4_drift_calibration_manifest_v2"
_DRIFT_PAYLOAD_SCHEMA = "redknot_dsv4_drift_calibration_payload_v1"
_DRIFT_DIGEST_SCOPE = {
    "algorithm": "sha256",
...
def _drift_capture_tensor_bytes(
    *, num_segments: int, sampled_rows: int, o_lora_rank: int
    ...
    row_instances_per_rank = num_segments * sampled_rows * (num_segments + 1)
    return (
        TP_SIZE
        * 43
        * row_instances_per_rank
        * 8  # logical heads per TP rank
        * o_lora_rank
...
def _run_projected_head_drift_profile() -> None:
    ...
    run_id = f"dsv4drift-{int(time.time())}-{secrets.token_hex(4)}"
    ...
    if num_layers != 43 or num_heads != 64 or o_lora_rank <= 0:
        raise ValueError(
            "drift calibration requires the 43-layer/64-head DeepSeek V4 "
            f"geometry with positive o_lora_rank, got layers={num_layers}, "
            f"heads={num_heads}, o_lora_rank={o_lora_rank}"
        )
    ...
    output_dir = Path(
        DRIFT_OUT_DIR or f"/tmp/redknot_mla_head_drift_{run_id}"
    ).expanduser().resolve()
    ...
    print(f"[drift] nested 8/16/24/32/48/56-head candidates: {analysis_dir}")
...
def _ih_performance_claim_qualification(
    *,
    ...
    A short diagnostic run is still useful for correctness.  In non-strict
    mode it is allowed to finish, but its result is explicitly ineligible for
    any speedup/QPS claim.  Strict mode rejects the run before model launch.
    """
...
def _ih_validate_exact_pure_plan(plan: Mapping[str, object], *, mode: str) -> None:
    ...
    if mode == "restore":
        ...
        if IH_COMBINED_HEADSPLIT_ROW_SPARSE:
            ...
            if (
                plan.get("mla_off_diagnostic_ablation") != "zoff_only"
                or plan.get("selection_policy") != "checkpoint_islands"
            ):
                raise ValueError("combined restore policy is not zoff/checkpoint")
...
def _ih_build_context_restore_plan(
    ...
    *,
    ...
    if IH_COMBINED_HEADSPLIT_ROW_SPARSE:
        ...
        plan.update(
            {
                "mla_off_diagnostic_ablation": "zoff_only",
                "skip_forward": True,
    ...
    *,
    ...
    if (len(chunks), int(chunk_tokens)) not in supported_geometry:
        raise ValueError(
            ...
            "8x32768, 16x32768, 8x56320, or 8x65536 geometry"
        )
...
def _ih_reproducibility_manifest(
    ...
    source_paths = {
        ...
        "server_launcher": REPO / "server/start_server_redknot_mla.sh",
    }
    source_files = {
        name: {
            "path": str(path),
    ...
    *,
    ...
    if (
        ...
        if full_q_rows > 0:
            ...
            # reconstructs the exact logical 64-head numerator; without this
...
def _ih_mla_off_capacity_geometry(total_segment_tokens: int) -> dict:
    ...
    if head_cfg_path:
        ...
        # classify only the middle layers and cannot weaken layers 0..2/40..42.
    ...
    if num_layers != 43 or dense_prefix != 3 or dense_suffix != 3:
        raise ValueError(
            "pure MLA headsplit capacity requires 43 layers with 3/3 dense fences"
        )
    ...
    for layer_id in range(num_layers):
        ...
        if 3 <= layer_id < 40 and (not local_groups or not global_heads):
            raise ValueError(
    ...
    expected_offline_layers = list(range(3, 40))
    if any(layer_ids != expected_offline_layers for layer_ids in local_layers_by_rank):
        raise ValueError(
            "pure MLA headsplit requires every TP rank to cache exactly layers 3..39"
        )
...
def _ih_verify_runtime_server_policy(
    ...
    if not (
        (capability_major == 8 and capability_minor == 9)
        or capability_major >= 9
    ):
        raise ValueError(
            "runtime MLA-off batch restore provider requires SM89 or SM90+"
        )
...
def _run_indexer_hot_benchmark():
    ...
    if IH_MLA_OFFLOAD and (
        ...
        raise ValueError(
            ...
            "16x8192, 8x32768, 16x32768, 8x56320, or 8x65536 offline "
            "document tokens"
        )
    ...
    if not IH_MLA_OFFLOAD:
        ...
        if not 1 <= IH_CHECKPOINT_MAX_ISLANDS <= 64:
            raise ValueError("checkpoint max islands must be in [1, 64]")
    ...
    try:
        ...
        formal_runtime_pass = bool(
            runtime_pass
            and not IH_MLA_OFF_DIAGNOSTIC_ONLY
            and not IH_MLA_OFF_QUALIFICATION_ONLY
        )
        if IH_MLA_OFF_DIAGNOSTIC_ONLY or IH_MLA_OFF_QUALIFICATION_ONLY:
        ...
        overall_pass = bool(
            not IH_MLA_OFF_DIAGNOSTIC_ONLY
            and not IH_MLA_OFF_QUALIFICATION_ONLY
            and quality_pass
        ...
        print(
            " full_model_mla_head_row_saving uses all model layers, all 64 "
        ...
        if IH_MLA_OFF_QUALIFICATION_ONLY:
            result_claim_status = "qualification_only"
            result_claim_reason = _IH_MLA_OFF_QUALIFICATION_CLAIM_REASON
        elif IH_MLA_OFF_DIAGNOSTIC_ONLY:
            result_claim_status = "diagnostic_only"
            result_claim_reason = _IH_MLA_OFF_DIAGNOSTIC_CLAIM_REASON
        else:
        ...
        result = {
            "test": (
                ...
                if IH_MLA_OFFLOAD
                    ...
                    f"redknot-dsv4-flash-{len(chunks)}x{chunk_tokens}-"
            ...
            "config": {
                ...
                "indexer_online_semantics": (
                    "restore_k_and_state_recompute_query_q_score_top512"
                    if IH_MLA_OFFLOAD
                    else None
                ),
                ...
                "mla_offline_online_layer_ids": (
                    list(range(3, 40)) if IH_MLA_OFFLOAD else []
                ),
                "design_targets": {
                    "ttft_speedup": 5.0,
            ...
            "runtime": {
                **runtime_metrics,
                ...
                "diagnostic_runtime_pass": (
                    runtime_pass if IH_MLA_OFF_DIAGNOSTIC_ONLY else None
                ),
            ...
            "performance_gate": {
                ...
                "diagnostic_only": IH_MLA_OFF_DIAGNOSTIC_ONLY,
                "qualification_only": IH_MLA_OFF_QUALIFICATION_ONLY,
                "status": result_claim_status,
...
def _run_sglang_engine_benchmark():
    ...
    W = 108
    print("=" * W)
    print(" BENCHMARK: DeepSeek V4 Flash RAG")
    print(f" Model: {MODEL_PATH}")
    print(
...
def main():
    ...
    if ENGINE_MODE == "indexer_hot" and not DRY_RUN:
        if not _run_indexer_hot_benchmark():
            strategy_label = (
            ...
            raise SystemExit(
                ...
                "quality/runtime/performance gate failed"
            )
        return
