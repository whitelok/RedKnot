...
DRIVER = REPO / "test/srt/redknot/utils/benchmark_RedKnot_DeepSeekV4_Flash_RAG.py"
MODEL = Path(
...
# B300 keeps the H200 8x64K release geometry intact and opts into a smaller
# physical snapshot unit for the 512K stress case.  The total logical context
# and all RedKnot head aggregation semantics are unchanged.
B300_TARGET_PROFILE_OVERRIDES = {
    524288: {
        "num_chunks": 16,
        "chunk_tokens": 32768,
        # 512K projects to about 37.0 GiB/rank.  A 42 GiB device cap keeps the
        # bank resident on the 267 GiB B300 while preserving explicit headroom.
        "mla_off_device_max_bytes": 45097156608,
    },
}
...
def _resolve_redknot_target_profile(args) -> dict:
    ...
    if os.environ.get("REDKNOT_HARDWARE_PROFILE", "").strip().lower() == "b300":
        profile.update(B300_TARGET_PROFILE_OVERRIDES.get(args.target_tokens, {}))
...
def _configure_redknot(
    ...
    _set(
        ...
        os.environ.get("REDKNOT_MOE_RUNNER_BACKEND", "marlin"),
    )
    ...
    # Keep the selected runtime end-to-end.  H200 defaults to .venv_tf5 while
    # B300 uses the CUDA-12.9 .venv_sm103 environment; forcing the old path
    # here would silently reintroduce CUDA-12.8 kernels in the server process.
    _set(
        "REDKNOT_IH_VENV_PY",
        os.environ.get("REDKNOT_PYTHON", sys.executable),
    )
