        ...
        if server_args.dp_size == 1:
            ...
            for pp_rank in pp_rank_range:
                for tp_rank in tp_rank_range:
                    ...
                    _bare = os.environ.get("SGLANG_BARE_SUBPROCESS_LAUNCH") == "1"
                    # Keep device reindex (each rank pins its own GPU); only the
                    # memory_saver / numa context managers below are skipped in
                    # bare mode (they break spawned workers on this host).
                    with maybe_reindex_device_id(gpu_id) as gpu_id:
                        ...
                        if _bare:
                            # Bare launch: skip memory_saver / numa context managers
                            # that wrap proc.start() (they break spawned scheduler
                            # workers on this host). Verified equivalent to a plain
                            # mp.Process().start() which works here.
                            proc.start()
                        else:
                            with (
...
def _set_envs_and_config(server_args: ServerArgs):
    ...
    # Set mp start method
    _mp_method = os.environ.get("SGLANG_MP_START_METHOD", "spawn")
