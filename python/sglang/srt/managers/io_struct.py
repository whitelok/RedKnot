...
class GenerateReqInput(BaseReq):
    ...
    # RedKnot offline (RAG) segment ids for the true sparse attention path
    # under the "redknot" backend. Per request a list of segment-id strings
    # (or None to opt out); for a batch a list of such per-request lists.
    ...
    def _normalize_redknot_offline_segments(self, num):
        """Normalize RedKnot offline-segment plans for batch processing.

        Accepted forms:
          * None                       -> [None] * num (opt out)
          * ["sidA", "sidB"]           -> same segment list for every request
          * [["sidA"], None, ["sidB"]] -> already per-request (only expanded
                                          for parallel sampling)
        """
        seg = self.redknot_offline_segments
        if seg is None:
            self.redknot_offline_segments = [None] * num
        elif isinstance(seg, list) and (len(seg) == 0 or isinstance(seg[0], str)):
            self.redknot_offline_segments = [list(seg)] * num
        elif isinstance(seg, list):
            self.redknot_offline_segments = seg * self.parallel_sample_num
        else:
            raise ValueError(
    ...
    def __getitem__(self, i):
        ...
        sub = GenerateReqInput(
            ...
            redknot_offline_segments=(
                self.redknot_offline_segments[i]
