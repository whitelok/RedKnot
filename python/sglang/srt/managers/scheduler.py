...
def run_scheduler_process(
    ...
    # [DEBUG] Force per-rank stderr/stdout + faulthandler to a file so crashes
    # before configure_logger are never lost.
    import os as _os

    if _os.environ.get("SGLANG_RANK_LOG_DIR"):
        import faulthandler as _fh

        _p = _os.path.join(_os.environ["SGLANG_RANK_LOG_DIR"], f"rank{tp_rank}.log")
        _f = open(_p, "w", buffering=1)
        _os.dup2(_f.fileno(), 1)
        _os.dup2(_f.fileno(), 2)
        _fh.enable(file=_f)
        _f.write(f"[rank{tp_rank}] run_scheduler_process ENTER gpu={gpu_id}\n")
        _f.flush()
    ...
    try:
        ...
        # DEBUG: also write per-rank exception to a file so multi-process
        # teardown cannot swallow it.
        try:
            with open(f"/tmp/sched_exc_rank{tp_rank}.txt", "w") as _f:
                _f.write(f"Scheduler rank {tp_rank} exception:\n{traceback}\n")
        except Exception:
            pass
