...
ATTENTION_BACKEND_CHOICES = [
    ...
    "torch_native",
    "flex_attention",
    "redknot",  # RedKnot: head-classified KV compression (see layers/attention/redknot_backend.py)
    "redknot_mla",  # RedKnot for DeepSeek V4 MLA logical heads
    "segpaged",  # SegPagedAttention: per-head paged KV + varlen attention
    "dsa",
...
class ServerArgs:
    ...
    # RedKnot (head-classified KV compression) — see
    # sglang/srt/layers/attention/redknot_backend.py
    # Path to a JSON describing per-(layer, kv_head) attention strategy.
    # When ``attention_backend == "redknot"`` and this is set, the backend
    # will load it on construction and route attention heads accordingly.
    redknot_head_config_path: Optional[str] = None
    # Query-axis chunk size for the RedKnot attention kernel. Kept for
    # API stability; the FA-2 kernel handles chunking internally so this
    # value mostly affects the prev-K/V realignment buffer.
    redknot_q_chunk_size: int = 2048
    # RedKnot attention kernel implementation: ``"fa2"`` uses the
    # ``flash_attn`` Python package (FA-2); ``"fa3"`` uses
    # ``sgl_kernel.flash_attn`` (FA-3) when the GPU supports it.
    redknot_kernel: str = "fa2"
    # DeepSeek V4 MLA RedKnot: token-selective FFN recovery. This is used with
    # ``attention_backend == "redknot_mla"`` to combine logical-head attention
    ...
    # reuses DeepSeek V4's native indexer signal (per-query top-k selection
    # length, ``c4_topk_lengths_raw``): tokens whose indexer retrieved more
    # global context are kept dense; weakly-engaged tokens are sparsified.
    # ``"blend"`` multiplies the two (indexer-mass-weighted activation norm).
    ...
    redknot_mla_profile_enable: bool = False
    redknot_mla_profile_out: Optional[str] = None
    redknot_mla_profile_coverage: float = 0.95
    redknot_mla_profile_sample_queries: int = 256
    redknot_mla_profile_global_window_ratio: float = 0.5
    redknot_mla_profile_window_safety: float = 1.5
    # SegPagedAttention decode: when True, decode reads a per-head paged KV
    # view (local heads keep only sink+recent) and runs the mask-free fused
    # varlen path instead of dense SDPA. See layers/attention/redknot/segpaged.py.
    redknot_segpaged_decode: bool = False
    redknot_page_size: int = 64

    # SegPagedAttention standalone backend — see
    # sglang/srt/layers/attention/segpaged_backend.py. These mirror the
    # RedKnot options but are used when ``--attention-backend segpaged``.
    segpaged_head_config_path: Optional[str] = None
    segpaged_q_chunk_size: int = 2048
    segpaged_kernel: str = "fa2"
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--redknot-q-chunk-size",
            type=int,
            default=ServerArgs.redknot_q_chunk_size,
            help="Query-axis chunk size for RedKnot attention (controls peak VRAM).",
        )
        parser.add_argument(
            "--redknot-kernel",
            type=str,
            default=ServerArgs.redknot_kernel,
            choices=["fa2", "fa3"],
            help=(
                "RedKnot attention kernel implementation. 'fa2' uses the "
                "flash_attn package; 'fa3' uses sgl_kernel.flash_attn (FA-3, "
                "Hopper-class GPUs)."
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            type=int,
            default=ServerArgs.redknot_sparse_ffn_dense_until,
            help="Keep FFN dense for layers below this index when sparse FFN is enabled.",
        )
        parser.add_argument(
            "--redknot-sparse-ffn-mass-thresh",
            type=float,
            default=ServerArgs.redknot_sparse_ffn_mass_thresh,
            help="Top-p mass threshold for mid-layer token selection in sparse FFN.",
        )
        parser.add_argument(
            "--redknot-sparse-ffn-deep-start",
            type=int,
            default=ServerArgs.redknot_sparse_ffn_deep_start,
            help="Layer index where the deep sparse-FFN threshold starts.",
        )
        parser.add_argument(
            "--redknot-sparse-ffn-mass-thresh-deep",
            type=float,
            default=ServerArgs.redknot_sparse_ffn_mass_thresh_deep,
            help="Top-p mass threshold for deep-layer token selection in sparse FFN.",
        )
        parser.add_argument(
            "--redknot-sparse-ffn-recent-n",
            type=int,
            default=ServerArgs.redknot_sparse_ffn_recent_n,
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "'activation' = post-attention L2 norm heuristic (default); "
                "'indexer' = reuse DeepSeek V4 indexer top-k selection length "
                "(c4_topk_lengths_raw, out-degree) so the same indexer signal "
                "drives both attention recompute and MoE; 'indexer_indegree' = "
                "page popularity (how many queries selected each token's c4 "
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=ServerArgs.redknot_mla_profile_enable,
            help="Enable the RedKnot MLA per-(layer,head) attention-locality "
            "profiler during prefill (exports head-class + concentration JSON).",
        )
        parser.add_argument(
            "--redknot-mla-profile-out",
            type=str,
            default=ServerArgs.redknot_mla_profile_out,
            help="Output path for the RedKnot MLA profiler JSON.",
        )
        parser.add_argument(
            "--redknot-mla-profile-coverage",
            type=float,
            default=ServerArgs.redknot_mla_profile_coverage,
            help="Attention-mass coverage target for head window sizing (e.g. 0.95).",
        )
        parser.add_argument(
            "--redknot-mla-profile-sample-queries",
            type=int,
            default=ServerArgs.redknot_mla_profile_sample_queries,
            help="Number of query rows sampled per prefill for the MLA profiler.",
        )
        parser.add_argument(
            "--redknot-mla-profile-global-window-ratio",
            type=float,
            default=ServerArgs.redknot_mla_profile_global_window_ratio,
            help="A head is 'global' if its coverage window exceeds this fraction "
            "of the context length.",
        )
        parser.add_argument(
            "--redknot-mla-profile-window-safety",
            type=float,
            default=ServerArgs.redknot_mla_profile_window_safety,
            help="Safety multiplier applied to the measured coverage window.",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            type=int,
            default=ServerArgs.redknot_mla_local_window,
            help="Local-head SWA window size for RedKnot MLA.",
        )
        parser.add_argument(
            "--redknot-mla-global-head-stride",
            type=int,
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=ServerArgs.redknot_segpaged_decode,
            help=(
                "Use SegPagedAttention for decode: per-head paged KV (local "
                "heads keep only sink+recent) + mask-free fused varlen "
                "attention, instead of dense SDPA over the full KV."
            ),
        )
        parser.add_argument(
            "--redknot-page-size",
            type=int,
            default=ServerArgs.redknot_page_size,
            help="SegPagedAttention page size (tokens per page) for decode.",
        )
        # SegPagedAttention standalone backend options.
        parser.add_argument(
            "--segpaged-head-config-path",
            type=str,
            default=ServerArgs.segpaged_head_config_path,
            help=(
                "Path to a head classification JSON for --attention-backend "
                "segpaged. If omitted, --redknot-head-config-path is used."
            ),
        )
        parser.add_argument(
            "--segpaged-q-chunk-size",
            type=int,
            default=ServerArgs.segpaged_q_chunk_size,
            help="Query-axis chunk size for SegPagedAttention prefill/extend.",
        )
        parser.add_argument(
            "--segpaged-kernel",
            type=str,
            default=ServerArgs.segpaged_kernel,
            choices=["fa2", "fa3"],
            help="Kernel used by the RedKnot prefill path under SegPagedAttention.",
        )
        parser.add_argument(
            "--segpaged-page-size",
            type=int,
            default=ServerArgs.segpaged_page_size,
            help="SegPagedAttention page size (tokens per page).",
        )
        parser.add_argument(
            "--sampling-backend",
            type=str,
