...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    # ── RedKnot hooks ────────────────────────────────────────────
    # Per-request list of offline segment ids (see
    # sglang.srt.layers.attention.redknot.OfflineKVCache). When set and the
    # active attention backend is "redknot", the backend splices these
    # offline pre-filled KV blocks into the prev-KV view with RoPE
    # realignment. ``None`` (or an inner ``None``) opts a request out and
    # falls back to plain SDPA semantics.
    redknot_offline_segments: Optional[List[Optional[List[str]]]] = None
    # Optional per-step HeadClassConfig override. Leave unset to use the
    # one bound to the backend at server start.
    redknot_head_config: Optional[object] = None
    # RedKnot DSV4 offline MLA reuse plan (demo). A dict describing whether the
